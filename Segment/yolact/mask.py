...
def evaluate_image(net_yolact, path: str, save_path: str):
    with torch.no_grad():
        cfg.mask_proto_debug = False
        img = cv2.imread(path)
        ...
        preds = net_yolact(batch)
        mask = prep(preds, img, None, None, undo_transform=False)
        ...
        if args.task == 'mask':
            ...
            for channel in range(3):
                img[:, :, channel] = mask.type(torch.uint8).cpu().numpy() \
                                     * img[:, :, channel]
                mask = img
...
def evaluate_images(net_yolact, path: str, save_path: str):
    ...
    for filename in os.listdir(path):
        evaluate_image(net_yolact, os.path.join(path, filename), os.path.join(save_path, filename))
...
def run(args):
    ...
    if args.video is not None:
        ...
        evaluate_image(net_yolact, inp, out)
